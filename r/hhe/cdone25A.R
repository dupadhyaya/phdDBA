...
#summarise------
cdoneS1 <- cdone2 %>% group_by(cDate) %>% summarise(n=n(), batcount = paste(batchID, collapse = ", "), duration = max(dur_min, na.rm=T), students = sum(pcount, na.rm=T)) %>% arrange(desc(cDate))
